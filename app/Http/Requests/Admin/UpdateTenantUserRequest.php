...
class UpdateTenantUserRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $tenantUserId = (int) $this->route('id');
        ...
        return [
            'tenant_uuid' => [$isGlobalSuper ? 'nullable' : 'nullable', 'string', 'exists:tenants,uuid'],
            ...
            'username' => [
                ...
                Rule::unique('tenant_users', 'username')
                    ->ignore($tenantUserId)
                    ->where(fn ($q) => $q->where('tenant_id', $tenantId)->whereNull('deleted_at')),
            ],
    ...
    private function tenantIdFromContext(): int
    {
        ...
        if ($isGlobalSuper) {
            $tenantUuidFromInput = trim((string) $this->input('tenant_uuid', ''));
            if ($tenantUuidFromInput !== '') {
                return (int) Tenant::query()->where('uuid', $tenantUuidFromInput)->value('id');
            }

            $tenantUserId = (int) $this->route('id');
            if ($tenantUserId > 0) {
                return (int) DB::table('tenant_users')->where('id', $tenantUserId)->value('tenant_id');
            }
        }
